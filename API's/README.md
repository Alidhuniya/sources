# API's

# Topics

web and web-services
internet
what is OSAP and API
xml and json
what is route,endpoint,parameter,schema
APi and types
Rest API and Restful API
xmlhttprequest
fetch api
async await
cross origin policy
mixed contents
Proxies
How to hide API's
Rate limiting
How to make your own API
Headless cms, Decoupled cms, Traditional CMS
GraphQL


# Youtube



# Blogs/article

## [Build Public API with github Pages](https://towardsdatascience.com/using-github-pages-for-creating-global-api-76b296c4b3b5)
## [ecmascript-asyncawait](https://tc39.es/ecmascript-asyncawait/)
## [cors-why-is-cors-so-important-for-web-development](https://dev.to/hemant/understand-cors-why-is-cors-so-important-for-web-development-46m8)
## [hiding-api-keys-in-your-code](https://dev.to/ivana_croxcatto/hiding-api-keys-in-your-code-1h0a?fbclid=IwAR1kboicyqkgUAGDwe475q1f5X5p7jXtEV_fCgJaeLzA8xuJIR-DtvUU1Z0)
## [Why you shouldn’t use google.com to search in Google](https://www.youtube.com/watch?v=U_RKc2UoMTY)
## [hsts](https://https.cio.gov/hsts/)
## [User Experience vs API - What Comes First?](https://www.youtube.com/watch?v=cpHPOkqyAk0)
## [When to use GraphQL over REST?](https://www.youtube.com/watch?v=J5NkUQ7XRYA)
## [When to use HTTP GET vs POST?](https://www.youtube.com/watch?v=K8HJ6DN23zI)
## [xmlhttp request vs fetch](https://www.youtube.com/watch?v=Vj7W8pI-L6w)
## [understanding-using-rest-ap](https://www.smashingmagazine.com/2018/01/understanding-using-rest-api/)
## [how-to-use-an-api](https://rapidapi.com/blog/how-to-use-an-api/)


# How to hide your API's
## [api-security-best-practices](https://dev.to/bearer/api-security-best-practices-3gjl?utm_source=digest_mailer&utm_medium=email&utm_campaign=digest_email)
## [twitter-discussion-hide-api](https://twitter.com/kvncnls/status/1309927434695913472)

## [Hiding API Keys in Your Code](https://dev.to/ivana_croxcatto/hiding-api-keys-in-your-code-1h0a?fbclid=IwAR1kboicyqkgUAGDwe475q1f5X5p7jXtEV_fCgJaeLzA8xuJIR-DtvUU1Z0)
## [env variable example](https://www.youtube.com/watch?v=PB5RQk5gAdQ)
## [problem with env. api-token-with-env-variable-is-not-secure-get-access-token-easily-of-contentful-project-with-react](https://dev.to/alidhuniya/api-token-with-env-variable-is-not-secure-get-access-token-easily-of-contentful-project-with-react-3ok8)
## [env variable](https://devpuskar.hashnode.dev/how-to-hide-your-api-keys-in-react-ckawllia2005jl6s12f24dk6b)
## [Hiding API Keys in Your Code](https://dev.to/ivana_croxcatto/hiding-api-keys-in-your-code-1h0a?fbclid=IwAR1kboicyqkgUAGDwe475q1f5X5p7jXtEV_fCgJaeLzA8xuJIR-DtvUU1Z0)
## [cloudfare](https://bigcodenerd.org/add-cloudflare-workers-subdomain-website/)
## [serverless](https://dev.to/nunocpnp/protecting-api-keys-with-serverless-functions-2ck7)
## [proxies](https://blog.mauriciorobayo.com/how-to-hide-your-front-end-api-keys-ck6jqvll900y3dfs11u3x96tn)
## [on server](https://www.youtube.com/watch?v=OlivFf6LdcE&list=PL-ZW1unRt6AvLhlqMWPry2KrBt2hPXOv5&index=119&t=26s)